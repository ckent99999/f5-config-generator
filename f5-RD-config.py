...
NODES = (
        'create ltm node /RPC/%(node_name)s { address %(container_address)s%(routedomain)s }'
)
...
VIRTUAL_ENTRIES = (
    ...
    ' destination %(internal_lb_vip_address)s%(routedomain)s:%(port)s'
...
PUB_SSL_VIRTUAL_ENTRIES = (
    'create ltm virtual /RPC/%(vs_name)s {'
    ' destination %(ssl_public_ip)s%(routedomain)s:%(port)s ip-protocol tcp'
    ' pool /RPC/%(pool_name)s'
...
SEC_HOSTNET_VIRTUAL_ENTRIES = (
    ...
    ' destination %(sec_host_net)s%(routedomain)s:0 ip-forward mask %(sec_host_netmask)s'
    r' profiles replace-all-with { /Common/fastL4 { } }'
...
SEC_CONTAINER_VIRTUAL_ENTRIES = (
    ...
    ' connection-limit 1 destination %(sec_container_net)s%(routedomain)s:0 ip-forward mask'
...
def args():
    ...
    parser.add_argument(
        '--routedomain',
        help='f5 route domain used for RPC nodes, virtuals, NATs',
        required=False,
        default="",
        #action='store_true'
    )
...
####
        ...
        for node in value['hosts']:
            if not user_args['routedomain']:             
                node['node_name'] = '%s_NODE_%s' % (PREFIX_NAME, node['hostname'])
                node['routedomain'] = ""
            else:
                node['node_name'] = '%s_NODE_%s' % (PREFIX_NAME, node['hostname'])
                node['routedomain'] = '%%%%%s'%user_args['routedomain']
####
            ...
            bigroutedomain=""
            if user_args['routedomain']:             
                bigroutedomain = '%%%s'%user_args['routedomain']
    ...
    if user_args['sec_host_network']:
        ...
        script.append(
            SEC_HOSTNET_VIRTUAL_ENTRIES % {
                ...
                'routedomain': '%%%s'%user_args['routedomain'],
            }
        )
